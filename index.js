// membuat variabel untuk menampung elemen DOM HTML
...
// inisialisasi untuk local storage
...
window.addEventListener("load", function () {
  if (typeof Storage !== "undefined") {
    // (MATERI 1)
    // inisialisasi semua item web storage yang akan kita gunakan jika belum ada
  ...
  //   (MATERI 2)
  //   inisialisasi semua nilai field pada dokumen menggunakan nilai dari web storage
  ...
  //   (MATERI 3)
  //   membuat event ketika tombol playButton di klik
  playButton.addEventListener("click", function () {
    sessionStorage.setItem(sessionAnswerKey, getAnswer());
    sessionStorage.setItem(sessionUserIsPlayingKey, true);
    beforeGameDisplay.hidden = true;
    duringGameDisplay.removeAttribute("hidden");
    duringGameDisplay.style.display = "flex";
  });

  //   membuat event ketika tombol answer button di klik
  ...
  //   Membuat fungsi untuk mengecek setiap jawaban yang dihasilkan
  ...
  window.addEventListener("beforeunload", function () {
    ...
    sessionStorage.setItem(sessionUserAttemptsKey, 0);
    sessionUserAttemptsField.innerText = this.sessionStorage.getItem(
      sessionUserAttemptsKey
    );
  });
});
